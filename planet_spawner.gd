...
func new_planet_position(radius: float) -> Variant:
    ...
    bounds.position = bounds.position + bounds_margin
    bounds.size = bounds.size - bounds_margin * 2
    ...
    for try in range(50_000):
        var offset = Vector2(randi_range(0, bounds.size.x), randi_range(0, bounds.size.y))
        var position = bounds.position + offset

        # If position is in an empty space on the map, return it
        var collides = planets.any(func(planet): return planet.position.distance_to(position) < radius + planet.radius + 70)
        if not collides:
            return position
